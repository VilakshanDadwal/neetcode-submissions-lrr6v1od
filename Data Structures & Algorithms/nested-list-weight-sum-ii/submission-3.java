 ...
 * }
 */
// DFS solution - with maxDepth derived from recursive function
class Solution {
    public int depthSumInverse(List<NestedInteger> nestedList) {
        var resultSum = 0;
        var depths = new ArrayList<int[]>();
        // Create list of depths.
        var maxDepth = findDepths(nestedList, 1, depths);
    ...
    private int findDepths(List<NestedInteger> nestedList, int depth, List<int[]>depths) {
        var maxDepth = depth;
        for(NestedInteger nestedObj : nestedList) {
            if(nestedObj.isInteger()) {
                depths.add(new int[] {nestedObj.getInteger(), depth});
            } else {
                maxDepth = findDepths(nestedObj.getList(), depth+1, depths);
            }
        }
        return maxDepth;
    }
