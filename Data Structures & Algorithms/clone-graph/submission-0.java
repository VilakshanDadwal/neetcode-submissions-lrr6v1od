...
class Solution {
    ...
    Node dfs(Node originalNode, Map<Node, Node> mapping) {
        ...
        for(Node orignalNeighbours : originalNode.neighbors) {
            var newNeighbours = newNode.neighbors;
            var newNeighbourNode = dfs(orignalNeighbours, mapping);
            newNeighbours.add(newNeighbourNode);
        }
        return newNode;
    }
}