...
class Solution {
    ...
    private TreeInfo diameter(TreeNode node) {
        if(node == null)
            // Using -1 as height so that for leaf node , height is 0.
            return new TreeInfo(0,-1);
        var leftInfo = diameter(node.left);
        var rightInfo = diameter(node.right);
        // Because length of path is no of edges, path through root is 2 edges plus some of height of left and right subtree
