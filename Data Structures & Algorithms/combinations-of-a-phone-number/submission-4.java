class Solution {
    ...
    public List<String> letterCombinations(String digits) {
        ...
        combos(0, new StringBuilder(), digits, result);
        return result;
    }

    private void combos(int idx, StringBuilder current, String digits, List<String> result) {
        if(idx == digits.length()) {
            result.add(current.toString());
            return;
        }
        var chars = MAPPING.get(digits.charAt(idx));
        for(char c: chars) {
            current.append(c);
            combos(idx+1, current, digits, result);
